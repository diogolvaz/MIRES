package com.amsavarthan.social.hify.service;
...
import static com.amsavarthan.social.hify.MIRES.Transaction.Configuration.getMIRESMetadata;
...
public class UploadService extends Service {
    ...
    private void uploadImages(final int notification_id, final int index, final List<Images> imagesList, String currentUser_id, String description, ArrayList<String> uploadedImagesUrl) {
        ...
        Map<String, Object> MIRES_transaction = transactionInitiated(FirebaseAuth.getInstance().getUid(), null);

        String filename="HIFY_"+System.currentTimeMillis()+"_"+imagesList.get(index).getName();
        final StorageReference fileToUpload=FirebaseStorage.getInstance().getReference().child("post_images").child(filename);
        fileToUpload.putFile(imageUri,getMIRESMetadata((String) MIRES_transaction.get("transaction_id")))
                        ...
                        .addOnSuccessListener(uri -> {
                            ...
                            try {
                                if (!TextUtils.isEmpty(imagesList.get(index + 1).getOg_path())) {
                                    ...
                                    uploadPost(notification_id,currentUser_id,description,uploadedImagesUrl, MIRES_transaction,filename);
                                }
                            }catch (Exception e){
                                e.printStackTrace();
                                uploadPost(notification_id,currentUser_id,description,uploadedImagesUrl, MIRES_transaction,filename);
    ...
    private void uploadPost(int notification_id, String currentUser_id, String description, ArrayList<String> uploadedImagesUrl, Map<String, Object> MIRES_transaction, String filename) {
            ...
            */
            ...
            FirebaseFirestore.getInstance().collection("Users")
                    ...
                    .addOnSuccessListener(documentSnapshot -> {
                        ...
                        all_reads.add(setDataRead(getVersionFromDocument(documentSnapshot),"Users/"+currentUser_id,read));

                        Map<String, Object> flag_object = getFlag(postMap, "Posts/" + post.getId(), "create", all_reads,MIRES_transaction);
                        ...
                        // Add content to the operation
                        batch.set(post, postMap);

                        // Add content to flag
                        batch.set(flag, flag_object);
